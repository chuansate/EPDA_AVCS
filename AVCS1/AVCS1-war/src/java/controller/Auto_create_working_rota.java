...
@WebServlet(name = "Auto_create_working_rota", urlPatterns = {"/Auto_create_working_rota"})
public class Auto_create_working_rota extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String year_rotaStr = request.getParameter("auto_year_rota");
        String month_rotaStr = request.getParameter("auto_month_rota");
        String day_rotaStr = request.getParameter("auto_day_rota");
        System.out.println("year rota = " + year_rotaStr);
        int year_rota;
        int month_rota;
        int day_rota;
        boolean invalidDate = false;
        boolean dateNotMonday = false;
        boolean duplicateStartingDateRota = false;
        ...
        try {
            ...
            // to catch the duplicate monday date
            List<WorkingRota> allWorkingRotas = workingRotaFacade.findAll();
            for (WorkingRota rota: allWorkingRotas) {
                if (rota.getStartingDate().getYear() == startingDateRota.getYear() && rota.getStartingDate().getMonth() == startingDateRota.getMonth() && rota.getStartingDate().getDayOfMonth() == startingDateRota.getDayOfMonth()) {
                    duplicateStartingDateRota = true;
                    break;
                }
            }
        } catch (Exception e) {
...
"            </table>");
            ...
            if (duplicateStartingDateRota) {
                out.println("The working rota for that week has been created before, try again!");
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
                List<WorkingRota> allWorkingRotas = workingRotaFacade.findAll();
            ...
            if (true) {  // it could be the returned conflict score is 0
                // add a new working rota to the table workingRota
                
                
                // get the number from the attributes of the Schedule obj, then instantiate the WorkingRota obj
...
//                workingRotaFacade.create(newWorkingRota);
                ...
                // otherwise, conflict score is more than 0, which asks the users to generate again!
                
                out.println("Failed to generate the working rota within the time limit, try again!");
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
                List<WorkingRota> allWorkingRotas = workingRotaFacade.findAll();
